...
2. **Шифрование персональных данных с использованием протокола HTTPS и хэшированием паролей**
3. **Использование современных механизмов аутентификации.**

## Надёжность
1. **Автосохранение данных в форме черновика должно срабатывать не реже, чем каждые 30 секунд после изменения, или при потере фокуса полем ввода.**
2. **Данные не должны теряться при обрыве соединения. Реализация очереди исходящих запросов на клиенте. При обрыве сети запросы ставятся в очередь и повторяются при восстановлении соединения. Критичные действия (например, отправка голоса) должны сопровождаться визуальным индикатором состояния ("Отправляется...", "Отправлено", "Ошибка сети - попытка повторной отправки").**
3. **Сервер должен обеспечивать стабильную работу при одновременной работой с каталогом от 5 000 помещений и загрузка до 20 документов (до 20 МБ каждый) на одно помещение без критического падения производительности.**

## Удобство использования
1. **Интерфейс должен быть интуитивно понятным, особенно для командной работы. Следовать общепринятым паттернам. Иметь единую систему визуальных компонентов. Ясные иконки и тексты кнопок. Иметь контекстные подсказки для неочевидных элементов. Четкое визуальное разделение информации по проектам и командам.**
2. **Поддержка удобного просмотра карточек помещений и комплексного сравнения вариантов.**
3. **Минимальное количество действий для голосования и обсуждения. Голосование "за/против/воздержался" — 1 клик. Добавление комментария к решению — не более 2 кликов (открыть поле ввода, отправить). Ответ на комментарий коллеги — 1 клик по кнопке "Ответить".**
